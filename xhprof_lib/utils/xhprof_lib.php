...
function init_metrics($xhprof_data, $rep_symbol, $sort, $diff_report = false)
{
    ...
    if (! empty($sort)) {
        if (array_key_exists($sort, $sortable_columns)) {
            ...
            echo "Invalid Sort Key {$sort} specified in URL";
        }
    }
 ...
 * @param array $functions_to_keep array of function names
 *
 ...
 *  @param   float $prune_percent any edges that account for less than
 ...
 * types of parameters.
 *
 * @param string $param name of the URL query string param
 */
 ...
 *
 * @param array $params An array whose keys are the names
