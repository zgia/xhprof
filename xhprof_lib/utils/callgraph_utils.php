 ...
 *
 * @param string $dot_script the script for DOT to generate the image
 * @param string $type       one of the supported image types, see $xhprof_legal_image_types
 *
 * @return string binary content of the generated image on success. empty string on
 *                failure.
 */
function xhprof_generate_image_by_dot($dot_script, $type)
{
    ...
    $cmd = 'dot -T' . $type;
    if (defined('XHPROF_DOT_CMD_PATH')) {
        $cmd = XHPROF_DOT_CMD_PATH . $cmd;
    }
 ...
 *
 * @param  mixed  $raw_data      phprof profile data
 ...
 *                               generated image.
 * @param  string $source        namespace for phprof database table
 * @param  string $page          the root node name. This can be used to
 *                               replace the 'main()' as the root node.
 * @param  string $func          the focus function
 * @param  bool   $critical_path whether or not to display critical path with
 *                               bold lines
 * @param  array  $right
 * @param  array  $left
...
function xhprof_generate_dot_script(
    ...
    foreach ($sym_table as $symbol => $info) {
        ...
        if ($left === null) {
            ...
            if (isset($left[$symbol], $right[$symbol])) {
 ...
 * @param mixed              $run1
 * @param mixed              $run2
 * @param mixed              $type             string, one of the supported image types. See also
 *                                             $xhprof_legal_image_types.
 * @param mixed              $threshold        float, the threshold value [0,1). The functions in the
 *                                             raw_data whose exclusive wall times ratio are below the
 *                                             threshold will be filtered out and won't apprear in the
 *                                             generated image.
 * @param string             $source           namespace for phprof database table
 ...
 * @param XHProfRuns_Default $xhprof_runs_impl An object that implements
 *                                             the iXHProfRuns interface
 * @param mixed              $run_id,          integer, the unique id for the phprof run, this is the
 ...
 *                                             threshold will be filtered out and won't apprear in the
 *                                             generated image.
 * @param string             $func,            string, the focus function
 * @param string             $source           namespace for phprof database table
 * @param mixed              $critical_path
 *
 * @return string the DOT script to generate image
 */
 ...
 * @param XHProfRuns_Default $xhprof_runs_impl An object that implements
 *                                             the iXHProfRuns interface
 * @param int                $run_id           the unique id for the phprof run, this is the
 *                                             primary key for phprof database table
 * @param string             $type             one of the supported image types. See also
 *                                             $xhprof_legal_image_types.
 * @param float              $threshold        the threshold value [0,1). The functions in the
 *                                             raw_data whose exclusive wall times ratio are below the
 *                                             threshold will be filtered out and won't apprear in the
 ...
 * @param string             $func             the focus function
 * @param string             $source           namespace for phprof database table
 * @param bool               $critical_path    does this run correspond to a PHProfLive run or a dev run?
 */
function xhprof_render_image(
    ...
    if (! $content) {
        echo 'Error: either we can not find profile data for run_id ' . $run_id
