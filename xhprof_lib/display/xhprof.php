 ...
 *
 * @param null|mixed $ui_dir_url_path
 */
 ...
 * @param mixed $sort
 * @param mixed $run1
 * @param mixed $run1_desc
 * @param mixed $run1_data
 * @param mixed $run2
 * @param mixed $run2_desc
 * @param mixed $run2_data
 */
function profiler_report(
    ...
    echo '<dl class=phprof_report_info>' .
...
function print_td_num($num, $fmt_func, $bold = false, $attributes = null)
{
    ...
    echo "<td {$attributes} {$class}>{$num}</td>\n";
}
...
function print_td_pct($numer, $denom, $bold = false, $attributes = null)
{
    ...
    echo "<td {$attributes} {$class}>{$pct}</td>\n";
}
...
function print_flat_data($url_params, $title, $flat_data, $sort, $run1, $run2, $limit)
{
    ...
    echo "<h3 align=center>{$title} {$display_link}</h3><br>";

    echo '<table border=1 cellpadding=2 cellspacing=1 width="90%" '
          . 'rules=rows bordercolor="#bdc7d8" align=center>';
    echo '<tr bgcolor="#bdc7d8" align=right>';
    ...
    foreach ($stats as $stat) {
        ...
        if ($stat == 'fn') {
            echo "<th align=left><nobr>{$header}</th>";
        } else {
            echo '<th ' . $vwbar . "><nobr>{$header}</th>";
        }
    }
    echo "</tr>\n";
...
function full_report($url_params, $symbol_tab, $sort, $run1, $run2)
{
    ...
    if ($diff_mode) {
        ...
        echo '<h3><center>Overall Diff Summary</center></h3>';
        echo '<table border=1 cellpadding=2 cellspacing=1 width="30%" '
              . 'rules=rows bordercolor="#bdc7d8" align=center>' . "\n";
        echo '<tr bgcolor="#bdc7d8" align=right>';
        echo '<th></th>';
        echo "<th {$vwbar}>" . xhprof_render_link("Run #{$run1}", $href1) . '</th>';
        echo "<th {$vwbar}>" . xhprof_render_link("Run #{$run2}", $href2) . '</th>';
        echo "<th {$vwbar}>Diff</th>";
        echo "<th {$vwbar}>Diff%</th>";
        echo '</tr>';
        ...
        foreach ($metrics as $metric) {
            $m = $metric;
            echo '<tr>';
            echo '<td>' . str_replace('<br>', ' ', $descriptions[$m]) . '</td>';
        ...
        echo '<table cellpadding=2 cellspacing=1 width="30%" '
...
function print_pc_array(
    ...
    echo '<b><i><center>' . $title . '</center></i></b>';
    echo '</td></tr>';
    ...
    foreach ($results as $info) {
        ...
        echo '<td>' . xhprof_render_link($info['fn'], $href);
        print_source_link($info);
        echo '</td>';
...
function print_symbol_summary($symbol_info, $stat, $base)
{
    ...
    echo ' (' . pct($val, $base) . '% of overall)';
    if (substr($stat, 0, 4) == 'excl') {
        $func_base = $symbol_info[str_replace('excl_', '', $stat)];
        echo ' (' . pct($val, $func_base) . '% of this function)';
    }
    echo '<br>';
}
 ...
 * @param mixed      $symbol_info
 * @param mixed      $sort
 * @param mixed      $rep_symbol
 * @param mixed      $run1
 * @param null|mixed $symbol_info1
 * @param mixed      $run2
 * @param null|mixed $symbol_info2
 */
function symbol_report(
    ...
    if ($diff_mode) {
        ...
        echo "<h3 align=center>{$regr_impr} summary for {$rep_symbol}<br><br></h3>";
        echo '<table border=1 cellpadding=2 cellspacing=1 width="30%" '
              . 'rules=rows bordercolor="#bdc7d8" align=center>' . "\n";
        echo '<tr bgcolor="#bdc7d8" align=right>';
        echo "<th align=left>{$rep_symbol}</th>";
        echo "<th {$vwbar}><a href=" . $href1 . ">Run #{$run1}</a></th>";
        echo "<th {$vwbar}><a href=" . $href2 . ">Run #{$run2}</a></th>";
        echo "<th {$vwbar}>Diff</th>";
        echo "<th {$vwbar}>Diff%</th>";
        echo '</tr>';
        echo '<tr>';
        ...
        foreach ($metrics as $metric) {
            ...
            echo '<tr>';
            echo '<td>' . str_replace('<br>', ' ', $descriptions[$m]) . ' per call </td>';
            $avg_info1 = 'N/A';
            $avg_info2 = 'N/A';
    ...
    echo '<br><h4><center>';
    echo "Parent/Child {$regr_impr} report for <b>{$rep_symbol}</b>";

    $callgraph_href = "{$base_path}/callgraph.php?"
      . http_build_query(xhprof_array_set($url_params, 'func', $rep_symbol));

    echo " <a href='{$callgraph_href}'>[View Callgraph {$diff_text}]</a><br>";
    ...
    echo "<td style='text-align:right;color:blue'>"
    ...
    echo "var func_name = '\"" . $rep_symbol . "\"';\n";
    echo 'var total_child_ct  = ' . $base_ct . ";\n";
    if ($display_calls) {
        echo 'var func_ct   = ' . $symbol_info['ct'] . ";\n";
    }
    echo "var func_metrics = new Array();\n";
    echo "var metrics_col  = new Array();\n";
    echo "var metrics_desc  = new Array();\n";
    if ($diff_mode) {
        echo "var diff_mode = true;\n";
    } else {
        echo "var diff_mode = false;\n";
    }
    $column_index = 3; // First three columns are Func Name, Calls, Calls%
    foreach ($metrics as $metric) {
        echo 'func_metrics["' . $metric . '"] = ' . round($symbol_info[$metric]) . ";\n";
        echo 'metrics_col["' . $metric . '"] = ' . $column_index . ";\n";
        echo 'metrics_desc["' . $metric . '"] = "' . $possible_metrics[$metric][2] . "\";\n";
